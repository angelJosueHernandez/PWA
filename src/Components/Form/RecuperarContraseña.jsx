...
export default function RecuperarContraseña() {
  ...
  const handleClick = async (direction) => {
    ...
    if (currentStep === 1 && direction === 'Siguiente') {
      if (!isMethodSelected) {
        message.error(
          'Por favor, seleccione un método de recuperación para continuar.',
        );
        ...
        if (correo == null) {
          ...
          fetch(
            `https://api-beta-mocha-59.vercel.app/recuperacionCorreo/${encodeURIComponent(correo)}`,
            {
              method: 'POST',
              headers: {
            ...
            .then(async (result) => {
              if (
                ...
                result.mensaje === 'Error al enviar el correo electrónico'
              ) {
                message.error(
                  ' Introduce un correo electrónico válido y funcional.',
                );
              }
            })
            .catch((error) => {
              ...
              setTimeout(() => {
                message.error(
                  'Error al enviar el Token. Por favor, intenta de nuevo más tarde.',
                );
              }, 900);
            });
        }
        ...
        if (correo == null) {
          ...
          fetch('https://api-beta-mocha-59.vercel.app/recuperacionSMS', {
            ...
            .then((result) => {
              if (
                result.mensaje === 'SMS de recuperación enviado correctamente'
              ) {
    ...
    if (currentStep === 2 && direction === 'Siguiente') {
      ...
      if (token == null) {
        message.error('El campo no puede estar vacío');
      } // Esperar a que verifyToken termine
      if (!tokenIsValid) {
    ...
    if (currentStep === 3 && direction === 'Siguiente') {
      if (contraActualizar === null && contraActualizar2 === null) {
        ...
        if (contraCo === false && contraCo2 === false) {
          ...
          fetch(
            `https://api-beta-mocha-59.vercel.app/actualizarContraRecuperacion`,
            {
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json',
            ...
            .catch((error) => {
              ...
              message.error(
                'Error al actualizar la contraseña, intenta de nuevo más tarde.',
              );
            });
        }
      }
    ...
    if (token == null) {
      ...
      try {
        const response = await fetch(
          `https://api-beta-mocha-59.vercel.app/compararToken/${encodeURIComponent(correo)}`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
